...
from ..core.utils import require_confirm, validate_string_not_empty
...
def register_docker_batch_tools(mcp: FastMCP) -> None:
    ...
    @mcp.tool()
    @tool_error_handler("update Docker containers")
    ...
    @mcp.tool()
    @tool_error_handler("update all Docker containers")
    async def update_all_docker_containers(
        ...
        """
        require_confirm(confirm, "update all containers")
    ...
    @mcp.tool()
    @tool_error_handler("update Docker autostart configuration")
