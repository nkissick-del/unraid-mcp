...
def register_array_tools(mcp: FastMCP) -> None:
    ...
    async def manage_array(desired_state: str) -> dict[str, Any]:
        ...
        """
        try:
            validated_state = validate_enum(
                desired_state.upper(), ["START", "STOP"], "desired_state"
            )
            logger.info(f"Executing manage_array: desired_state={validated_state}")
            ...
            logger.error(f"Error in manage_array: {e}", exc_info=True)
            raise ToolError(f"Failed to {validated_state.lower()} the array: {str(e)}") from e
