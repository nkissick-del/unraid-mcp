...
class TestSetThemeValidation:
    ...
    async def test_empty_config_raises(self):
        ...
        register_customization_tools(test_mcp)

        tool_fn = None
        for tool in test_mcp._tool_manager._tools.values():
            if tool.name == "set_theme":
                tool_fn = tool.fn
                break

        assert tool_fn is not None, "set_theme tool not registered"
...
class TestSetLocaleValidation:
    ...
    async def test_empty_locale_raises(self):
        ...
        tool_fn = None
        for tool in test_mcp._tool_manager._tools.values():
            if tool.name == "set_locale":
                tool_fn = tool.fn
                break

        assert tool_fn is not None, "set_locale tool not registered"
